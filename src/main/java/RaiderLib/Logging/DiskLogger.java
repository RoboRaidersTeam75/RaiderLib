  ...
  public void sendUpdates(LogTable logTable) {
    for (Map.Entry<String, LoggedValue> field : logTable.updatesMap().entrySet()) {
      ...
      switch (field.getValue().type) {
        ...
        // Not entirely sure if casting to double arrays will work like this
        case Pose2d:
          log.appendDoubleArray(id, (double[]) field.getValue().getLogValue(), logTable.timestamp()); // [x, y, θ]
          break;
        case SwerveModulePosition:
          log.appendDoubleArray(id, (double[]) field.getValue().getLogValue(), logTable.timestamp()); // [distance, θ]
          break;
        case SwerveModuleState:
          log.appendDoubleArray(id, (double[]) field.getValue().getLogValue(), logTable.timestamp()); // [speed, θ]
          break;
      }
    }
  }
